...
@section('content')
    <div class="col s12">
        <header class="row navigation-row">
            ...
            <div class="col s6">
                <a href="{{ url('/admin/technology/create') }}" class="waves-effect waves-light btn right"
                ...
                <div class="card">
                    ...
                    <div class="card-content">
                        <table class="responsive-table bordered striped highlight">
                            ...
                            <tbody class="sortable" data-entityname="technologies">
                            @foreach($technology as $item)
                                <tr>
                                    <td class="center sortable-handle"><span class="fa fa-sort"></span></td>
                                    <td class="center">{{ $loop->iteration }}</td><td>
                                        @if ($item->file != NULL)
                                            <img src="/files/thumb/technologies/100x100_{{ $item->file }}" class="news-image-small" />
                                        @endif
                                    </td>
                                    <td>{{ $item->title }}</td>
                                    <td class="center">
                                    ...
                                    <td>{{ $item->subtitle }}</td>
                                    <td>
                                        <a href="{{ url('/admin/technology/' . $item->id) }}"
                                           class="waves-effect waves-light btn"
                                           title="Zobacz"><i class="fa fa-folder-open"></i></a>
                                        <a href="{{ url('/admin/technology/' . $item->id . '/edit') }}"
                                           class="waves-effect waves-light btn"
